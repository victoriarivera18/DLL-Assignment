...
class DLList {
  ...
  public:
    DLList() : num(0), header(T()), trailer(T())  { header.next = &trailer; trailer.prev = &header; } // default constructor
...
template<typename T>
DLList<T>& DLList<T>::operator=(DLList<T>&& dll)  // ned to finish this
{
  if (this == &dll) {
    return *this;
  } else {
...
T DLList<T>::remove_after(DLListNode<T> &p)
{
  if (is_empty()){
    ...
    T obj;
    if (p.next == nullptr) {  // means its last node
      obj = remove_last();
    } else {
